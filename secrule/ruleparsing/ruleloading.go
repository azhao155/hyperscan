...
var ruleSetPathsMap = map[waf.RuleSetID][]string{
	...
	"OWASP CRS 3.2": {"crs3.2/main.conf"},
	"OWASP CRS 3.2 with config for regression tests": {"crs3.2/main.regressiontesting.conf"},
}
...
func getCrsRulesPath() string {
	...
	startedByDlv := strings.HasSuffix(execPath, "/debug") || strings.HasSuffix(execPath, "/debug.test") || strings.HasSuffix(execPath, "/__debug_bin")
